...
<section class="main">
	<article id="body">
		...
		<c:if test="${BODY == 'FILE_UP'}">
			<%@ include file="/WEB-INF/views/bodys/fileUp_form.jsp" %>
		</c:if>
		<c:if test="${BODY == 'LOGIN_FORM'}">
			<%@ include file="/WEB-INF/views/bodys/login_form2.jsp" %>
		</c:if>
		<c:if test="${BODY == 'FILES_UP'}">
			<%@ include file="/WEB-INF/views/bodys/filesUp_form.jsp" %>
		</c:if>
		<c:if test="${BODY == 'BBS_WRITE'}">
			<%@ include file="/WEB-INF/views/bodys/bbs_form.jsp" %>
		</c:if>
		<c:if test="${BODY == 'BBS_LIST'}">
			<%@ include file="/WEB-INF/views/bodys/bbs_list.jsp" %>
		</c:if>
