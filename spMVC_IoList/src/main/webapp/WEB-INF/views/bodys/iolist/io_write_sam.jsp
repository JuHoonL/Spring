...
$(function(){
	...
	$("#io_pcode").keydown(function(key){
		/* 글자를 입력 중에 Enter를 입력하면 이라는 조건문 */
		if(key.keyCode == 13){
			$("#myModal").css("display","block")
			let io_inout = $("#io_inout").val() // 매입매출구분
			let io_pname = $(this).val()
			$.get("${rootPath}/product/search?p_name=" + io_pname,function(result){
	...
	$("#io_dcode").keydown(function(key){
		...
		if(key.keyCode == 13){
			$("#myModal").css("display","block")
			let io_dname = $(this).val()
			$.get("${rootPath}/dept/search?d_name=" + io_dname,function(result){
				$("#modal-body").html(result)
			})
		}
	})
	...
	$("#io_quan").keydown(function(key){
		if(key.keyCode == 13) { 
			var io_quan = parseInt($(this).val())
			var io_price = parseInt($("#io_price").val())
			
			
			let sum = io_quan * io_price
			alert(sum)
			let io_tax_total = sum
			
			$("#io_total").val(sum)
			if($("input[name='io_tax']:checked").val() == '1') {
				io_tax_total *= 1.1
				io_tax_total = parseInt(io_tax_total)
			}
			if($("input[name='io_tax']:checked").val() == '0') {
				io_tax_total = sum
			}
...
<form:form action="${pageContext.request.contextPath}/iolist/write" modelAttribute="iolistVO">
	<fieldset>
		...
		<div class='in-box'>
			<form:input class="in-box2" readonly="true" id="io_price" path="io_price" /><br/>
			<form:errors path="io_price" class="in-box-error" />
		</div>
		
		<label class="in-label" for="io_total">공급가액</label>
		<div class='in-box'>
			<form:input class="in-box2" readonly="true" id="io_total" path="io_total" /><br/>
			<form:errors path="io_total" class="in-box-error" />
		</div>
		
		<label class="in-label" for="io_tax_total">합계</label>
		<div class='in-box'>
			<form:input class="in-box2" readonly="true" id="io_tax_total" path="io_tax_total" /><br/>
